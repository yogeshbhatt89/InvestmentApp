...
import {
  useRecommendationTrends,
  RecommendationTrend,
} from '@/services/finnhub/useRecommendationTrends'
...
const CompanyRecommendationTrendSection: React.FC<CompanyRecommendationTrendSectionProps> = ({
  symbol,
}) => {
    ...
    return (
      <CardWrapperComponent className="company-recommendation-trend-card">
        <BoxComponent sx={{ p: 2, opacity: 0.5 }}>
          <TypographyComponent variant="body1">Loading...</TypographyComponent>
        </BoxComponent>
      ...
      <BoxComponent sx={{ p: 2 }}>
        ...
        {data &&
          ...
          data.data
            ...
            .map((trend: RecommendationTrend) => (
              <CardWrapperComponent key={trend.period} className="recommendation-trend-card mb-2">
                <BoxComponent sx={{ p: 2 }}>
                  <TypographyComponent variant="h6" sx={{ mb: 2, fontWeight: 'bold' }}>
                    {trend.period}
                  </TypographyComponent>
                  <TypographyComponent variant="body2">Buy: {trend.buy}</TypographyComponent>
                  <TypographyComponent variant="body2">Hold: {trend.hold}</TypographyComponent>
                  <TypographyComponent variant="body2">Sell: {trend.sell}</TypographyComponent>
